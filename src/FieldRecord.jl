...
Available `selectargs` depend on the grid `fr.mesh` and `fr.data_dims`, options include:
- grid spatial dimensions 
[- TODO grid spatial coordinates (if defined)]
- `fr.data_dims`


Optional argument `coords` can be used to supply coordinates from additional `FieldRecords`, replacing any coordinates
attached to `fr`. Format is a Vector of Pairs of "dim_name"=>(cr1::FieldRecord, cr2::FieldRecord, ...)
...
function _get_array(
    fr::FieldRecord{FieldData, Space, V, N, Mesh, R}, allselectargs::NamedTuple, 
    ...
    for (k, v) in zip(keys(allselectargs), values(allselectargs))
        if k==:records
            if v isa Integer
                ridx = [v]                
            else
        ...
        elseif String(k) in fr.dataset.record_dim_coordinates
            # find ridx corresponding to a coordinate
            rfr = PB.get_field(fr.dataset, String(k))
            ridx, cvalue = find_indices(rfr.records, v)
            selectargs_records = NamedTuple((k=>cvalue,))    
        else
            selectargs_region = merge(selectargs_region, NamedTuple((k=>v,)))
        end
    end
    records_coords = FixedCoord[]
    for rcn in fr.dataset.record_dim_coordinates
        rfr = PB.get_field(fr.dataset, rcn)
        push!(records_coords, FixedCoord(rcn, rfr.records[ridx], rfr.attributes))
    end
    records_dim_coords = PB.NamedDimension(fr.dataset.record_dim.name, length(ridx)) => records_coords            
    ...
    if field_single_element(fr)
        ...
        data_dims_coords = get_data_dims_coords((fr.dataset, ridx), fr.data_dims)
        ...
        if length(ridx) > 1
            return FieldArray(
                name,
                fr.records[ridx], 
                vcat(data_dims_coords, records_dim_coords),
                attributes
            )
        else
        ...
        far = get_array(fr[first(ridx)], selectargs_region; coord_source=(fr.dataset, first(ridx)))
        ...
        if length(ridx) > 1
            ...
            favalues = Array{eltype(far.values), length(far.dims_coords)+1}(undef, size(far.values)..., length(ridx))
            fa = FieldArray(
                name,
                favalues, 
                vcat(far.dims_coords, records_dim_coords), 
                attributes,
            )
            # copy values for first record
            if isempty(far.dims_coords)
                fa.values[1] = far.values
            else
                fa.values[fill(Colon(), length(far.dims_coords))..., 1] .= far.values
            end
        else
        ...
        if length(ridx) > 1
            for (i, r) in enumerate(ridx[2:end])
                far = get_array(fr[r], selectargs_region; coord_source=(fr.dataset, r))
                if isempty(far.dims_coords)
                    fa.values[i+1] = far.values
                else
                    fa.values[fill(Colon(), length(far.dims_coords))..., i+1] .= far.values
                end
            end
        end
...
function get_array(
    ...
    @info "get_array (begin): $frname, allselectargs $allselectargs"
    ...
    @info "get_array (end): $frname -> $name, allselectargs $allselectargs"
