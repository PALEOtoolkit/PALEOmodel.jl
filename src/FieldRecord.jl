...
"""    
    get_array(fr::FieldRecord [, allselectargs::NamedTuple] [,coords_records::AbstractVector]) -> FieldArray
    get_array(fr::FieldRecord; [allselectargs...]) -> FieldArray
...
Optional argument `coords_records` can be used to supply coordinates from additional `FieldRecords`, replacing any coordinates
...
"""
function get_array(
    fr::FieldRecord; allselectargs...
)
    Base.depwarn(
        "allselectargs... will be deprecated in a future release.  Please use allselectargs::NamedTuple instead",
        :get_array,
    )
    return get_array(fr, NamedTuple(allselectargs))
end

function get_array(
    fr::FieldRecord{D, S, V, N, M, R}, allselectargs::NamedTuple=NamedTuple(),
) where {D, S, V, N, M, R}
...
# substitute or add coords_records to FieldArray(fr; selectargs...)
function get_array(fr::FieldRecord, allselectargs::NamedTuple, coords_records::AbstractVector)

    # get data NB: with original coords or no coords
    varray = get_array(fr; allselectargs...)

    # generate Vector of NamedDimensions to use as new coordinates
    named_dimensions = PB.NamedDimension[]
    for (coord_name, coord_fieldrecords) in coords_records
        fixed_coords = []
        for coord_fr in coord_fieldrecords
            coord_array = get_array(coord_fr; allselectargs...)
    ...
    # replace coordinates
    varray_coords = FieldArray(varray.name, varray.values, Tuple(named_dimensions), varray.attributes)

    return varray_coords
end