...
- `step_callbacks=[]`: callbacks on successful step, `[step_callback(sol.zero, tss, deltat, model, modeldata)]`, eg to project solution onto invariant manifold is to project the solution onto an invariant manifold [Shampine1999](@cite) 
- `use_norm=false`: not supported (must be false)
